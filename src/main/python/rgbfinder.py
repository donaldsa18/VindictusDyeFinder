...
class RGBFinder:
	def __init__(self, gui=None):
		...
		self.screencap_path = "resources\\screencap.bmp"
		self.gui = gui
		if gui is not None:
	...
	def get_screencap(self):
		...
		if not hwnd:
			img = cv2.imread(self.screencap_path)
			w = img.shape[1]
			h = img.shape[0]
			return img, w, h, None, None, None, None
		...
		dataBitMap.SaveBitmapFile(cDC, self.screencap_path)
	...
	def run(self):
		...
		self.log("Checking for colors")
		for color in self.colors:
			self.find_color(color, img)
			self.log("{} at ({},{}) {}".format(color[0], color[5], color[6], img[color[6], color[5]]))

		if x is not None and self.gui is None:
